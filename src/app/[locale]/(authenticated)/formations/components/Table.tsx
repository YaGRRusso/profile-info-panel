    ...
    <>
          ...
          {data?.data.map((formation) => (
            <Table.Row key={formation.id}>
              ...
              <Table.Cell aria-disabled={!formation.certificate}>
                <Link href={formation?.certificate ?? '#'} target="_blank" className="emphasis">
      ...
      >
        <FormationsCommonForm
          ...
          defaultValues={{
            ...editingFormation,
            start: editingFormation?.start ? new Date(editingFormation.start) : undefined,
            end: editingFormation?.end ? new Date(editingFormation.end) : undefined,
          }}
